...
sessionRouter.get('/sessions', (c) => {
  return c.json({ message: 'Sessions endpoint' });
});

sessionRouter.get('/sessions/:id', (c) => {
...
sessionRouter.post('/sessions/:id/join', (c) => {
  const { id } = c.req.param();
...
sessionRouter.post('/sessions/:id/terminate', (c) => {
  const { id } = c.req.param();
